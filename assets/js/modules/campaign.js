...
myValidate();
$('#file_name, #video_ads, #mobile_bid_mdifier, #language, #countries, #display_url, #destination_url, #budget, #max_cpv').change(myValidate);	
$('#video_ads').hide();
$('#video_ads_list').css({'margin-top':'10px', 'height':'220px', 'width':'520px', 'padding':'0px', 'list-style' : 'none', 'overflow-y' : 'auto', 'background-color' : '#eee' });
	...
	$('.disabled-detector').on('click',function(){
		//console.log($(this).prev('.target_checkbox').val());
		var checkbox= $(this).prev('.target_checkbox');
		var ytdata 	= parseInt($(this).parents('label').data('count'));
		var checked = ((checkbox.prop('checked')) ? -1 : 1);
        var add 	= ytdata * checked;
        var total 	= parseInt($("#total_target_vid font").html()) + add;
        var left    = $('.target_checkbox:checked').length;
        var stopped = false;
        var targetname=$(this).parents('label').data('name');
        
        if ( checkbox.prop('disabled') === false ) {
	        if ( checked < 0 ) {
	        	checkbox.prop('checked',false);
	        	left = $('.target_checkbox:checked').length;
		        $('.target_checkbox:not(:checked)').prop('disabled',false);
			    //$('.target_checkbox:not(:checked)').next('.disabled-detector').css('display','inline');
	        }
	        else {
	        	...
	        	if ( ytdata > max_tgv ) {
					if ( left > 1 ) {
		            	$('#target_limit_modal .modal-body p').html("This "+targetname+" list needs to be it’s OWN campaign, so untick all other lists and we’ll create an Adwords campaign JUST for this "+targetname+" list, "+$('#usrloginName').val()+"");
			            ...
			            //$('.target_checkbox:not(:checked)').next('.disabled-detector').css('display','none');
		            }
		            stopped = true;
		        }
		        ...
		        if ( !stopped ) {
			        if(total == 0) {
			        	...
			        	$('#target_list').css('border', 'medium solid #a94442');
			        }
			        else{
	    	...
	    	if ( left == 10 ) {
			    $('#target_limit_modal .modal-body p').html("MAXIMUM TARGET GROUPS REACHED! We only allow a maximum of 10 lists to be used in any one campaign.");
		    }
		    else {
		    	$('#target_limit_modal .modal-body p').html("Once you select a list this large "+$('#usrloginName').val()+", we’re going to create a campaign around JUST this list, split into multiple targeting groups in Adwords. Fancy way of saying “you can’t select any more lists with this "+targetname+" list selected, so now go export to Adwords at the bottom of the screen.");
		    }
	...
	$('.target_checkbox').on('click', function(e){
		e.preventDefault();
	});
...
*/
	...
	function SaveCampaign ( el, exported,from_export ) {

		if ( $(el).data('exported') == 1 ) {
			$('#exported_modal').modal('show');
		}
		else {
			...
			var disp_url 			= $('#display_url').val().trim().toLowerCase();
			var dest_url 			= $('#destination_url').val().trim().toLowerCase();
			...
			if(date_today > start_date_db){
				/*
$('#error-msg span').html('Set your campaign date to be TODAY in "Show Advanced Campaign Options"');
				ShowError(true);
*/
				show_loader ( 'ERROR', false, 100 );
				$('#date_today_modal').modal('show');
				return false;
			}
			
			if(parseFloat(budget) <= 0){
				$('#error-msg span').html('Budget must be greater than 0');
				ShowError(true);
				show_loader ( 'ERROR', false, 100 );
				return false;
			}
			...
			if(parseFloat(max_cpv) <= 0){
				$('#error-msg span').html('max cpv must be greater than 0');
				ShowError(true);
				show_loader ( 'ERROR', false, 100 );
				return false;
			}
				...
				$.ajax({
				  ...
				  dataType  : 'json'
					...
					if ( data.valid && data.method == 'add' ) {
						...
						//save campaign from optimizer
						if ( $("#token_key").length > 0 && $("#optimizer_id").length > 0 ) {
							var token_key 		= $("#token_key").val();
							var optimizer_id 	= $("#optimizer_id").val();
			
							if(token_key.length > 0 && optimizer_id != ""){
								save_campaign_from_optimizer(data.id,token_key,optimizer_id);
							}else{
	...
	function ExportCampaign ( el, dataToSend ) {
		var theButton = $(el);
		...
		if ($('.target_checkbox:checked').length > 1) {
			$('.target_checkbox:checked').each(function(y, val){
				//console.log(val);
				//var targeting_group = $(val).parent('label').text().replace(/\s+\(.+?\)/g, '');
				var targetgroupname  = $.trim($(val).parent('label').text().replace(/\s+\(.+?\)/g, ''));
				targetgroups.push(targetgroupname);
				...
				for(var k=0;k<gender.length;k++){
					...
					if(gender[k]!=null){
						...
						var target_g_content = "<tr class=\"table-data\"> <td>Add</td> <td>Gender</td> <td>Enabled</td> <td> "+$(gender[k]).val()+"</td> <td>"+targetgroupname+"</td> <td align= 'center'> #N/A </td> <td colspan=\"50\"></td> </tr>";
				...
				for(var l=0;l<age.length;l++){
					...
					if(age[l]!=null){
							...
							var target_a_content = "<tr class=\"table-data\"> <td>Add</td> <td>Age</td> <td>Enabled</td> <td> "+$(age[l]).val()+"</td> <td>"+targetgroupname+"</td> <td align= 'center'> #N/A </td> <td colspan=\"50\"> </td> </tr>";
				...
				var target_tg_content = "<tr class=\"table-data\"> <td>Add</td> <td>Enabled</td> <td>"+targetgroupname+"</td> <td>"+filename+"</td> <td>"+max_cpv+"</td> <td colspan=\"50\"></td></tr>";
	            ...
	            $.each(ytdata,function(key,value){
		            ...
		            var target_details_content = "<tr class=\"table-data\"> <td>Add</td> <td>Youtube video</td> <td>Enabled</td> <td>https://www.youtube.com/watch?v="+value['ytid']+"</td> <td>"+targetgroupname+"</td> <td>#N/A</td> <td colspan=\"50\"></td> </tr>";
			...
			$('.target_checkbox:checked').each(function(y, val){
	            ...
	            $.each(ytdata,function(key,value){
		            ...
		            var newGroup = targetgroupname+" - Group "+index;
		            
		            if ( !tgArr[newGroup] ) {
			            tgArr[newGroup] = [];
		            }
		            
		            tgArr[newGroup].push(value['ytid']);
		            dtgArr.push(value['ytid']);
					
		            var target_details_content = "<tr id='"+key+"' class=\"table-data\"> <td>Add</td> <td>Youtube video</td> <td>Enabled</td> <td>https://www.youtube.com/watch?v="+value['ytid']+"</td> <td>"+newGroup+"</td> <td>#N/A</td> <td colspan=\"50\"></td> </tr>";
	            ...
	            for (var i=0;i<=group_index;i++) {
		        	var index = i + 1;
		        	var split_targetname = targetgroupname+" - Group "+index;
		        	
					targetgroups.push(split_targetname);
					...
					for(var k=0;k<gender.length;k++){
						...
						if(gender[k]!=null){
							...
							var target_g_content = "<tr class=\"table-data\"> <td>Add</td> <td>Gender</td> <td>Enabled</td> <td> "+$(gender[k]).val()+"</td> <td>"+split_targetname+"</td> <td align= 'center'> #N/A </td> <td colspan=\"50\"></td> </tr>";
					...
					for(var l=0;l<age.length;l++){
						...
						if(age[l]!=null){
								...
								var target_a_content = "<tr class=\"table-data\"> <td>Add</td> <td>Age</td> <td>Enabled</td> <td> "+$(age[l]).val()+"</td> <td>"+split_targetname+"</td> <td align= 'center'> #N/A </td> <td colspan=\"50\"> </td> </tr>";
					...
					//Targeting Group Loop
					var target_tg_content = "<tr class=\"table-data\"> <td>Add</td> <td>Enabled</td> <td>"+split_targetname+"</td> <td>"+filename+"</td> <td>"+max_cpv+"</td> <td colspan=\"50\"></td></tr>";
		...
		//targetgroupslist = $.trim(targetgroups.join()).replace(/ /g,'');
		//targetgroupslist = $.trim(targetgroups.join()).replace(/\n/g,'');
		targetgroupslist = $.trim(targetgroups.join());
		//console.log(targetgroupslist);
		
		for(var j=0;j<video_ads.length;j++){
		
			if(video_ads[j]!=null){	
				//console.log($(video_ads_option[i]).data('title'));			
				var target_ads_content = "<tr class=\"table-data target-ads\"> <td>Add</td> <td>Enabled</td> <td>"+$(video_ads_option[j]).data('title')+"</td> <td>"+video_ads[j]+"</td> <td> default</td> <td>#N/A</td> <td> #N/A </td> <td>#N/A</td> <td>"+disp_url+"</td> <td>"+dest_url+"</td> <td>#N/A</td> <td>in-stream</td> <td>#N/A</td> <td>"+targetgroupslist+"</td> <td>"+filename+"</td> <td colspan=\"41\"></td></tr>";
		...
		$.each(dtgArr, function(key,value) {
		  if (!ntgObj.hasOwnProperty(value)) {
		    ntgObj[value] = 1;
		  } else {
		    ntgObj[value]++;
		  }
		...
		//exportTableToCSV.apply(el, [$('#csv_table>table'), filename+'.csv', dataToSend]);
        $("#aw-success").dialog({
        ...
        $("#aw-success").siblings('div.ui-dialog-titlebar').remove();
	}

	$('#saveCampaign').bind('click', function(){
		SaveCampaign($(this), false, 0);
	});

	$('#btnExportCSV').bind('click', function(){
		var saved = SaveCampaign($('#saveCampaign'), this,1);
		if (saved) {
			
		console.log(saved);

			ExportCampaign(this,saved);
		}
	});

	function exportTableToCSV($table, filename, dataToSend) {
...
*/
        ...
        $.ajax({
			...
			data: { data: filename, cid: $('#campaign_list').val(), cdata: dataToSend },
    ...
    function ClearForm() {
    	...
    	var export_count = $('#export_count').val();
/*    	if(export_count == 0){
    		show_loader( 'SAVING', false, 400 );
			...
			var cname = "first_export";
			document.cookie = cname + "=" + cvalue + "; " + expires;
			$('#first_export').modal('show');
    	}else{
				...
				//console.log(data);
			      if ( data.items.length > 0 ) {
				      ...
				      if(vidIdCollect.length > 0) {
						...
						//- check if optimized campaign
						if ( opt_id ) {
					        $("#campaign_list").val(opt_id);
					        get_campaign(opt_id , false, true );
						}
			      }
...
*/
	...
	function AddVideoAdsUrl (vidId, vidUrl, vidTitle, thumbnail, isSelected) {
        ...
        if(vidUrl != "") {
            var li = $('<li>');
            var button = $('<button class="btn btn-danger" type="button" style="margin-left:2em;">x</button>');
            ...
            li.html('<label class="glyphicon ' + selected + '" style="width: 20px;height: 20px;border: 2px solid #333;display: inline-block; vertical-align:middle; "></label><img src="' + thumbnail + '" style="margin-left:14px;display:inline-block" width="68" /><span class="glyphicon list-indent " style="width:60%; display:inline-block; font-size: 1em;">' + vidUrl + '</span>');
...
function Validation () {
	...
	else if(validateNumber($('#budget').val().trim()) <= 0 ) {//target list
		$('#error-msg span').html('Budget must be greater than 0.');
		return true;
	}
	else if(validateNumber($('#max_cpv').val().trim()) <= 0 ) {//target list
		$('#error-msg span').html('max cpv must be greater than 0.');
		return true;
	}
	return false;
...
function validateURL(url, method){
	...
	switch (method) {
		case 'dest':
			website_regex = /^(http:\/\/|http:\/\/www\.|https:\/\/|https:\/\/www\.)[a-zA-Z0-9]+([\-\.]{1}[a-zA-Z0-9]+)*\.[a-zA-Z]{2,3}(:[0-9]{1,5})?(\/.*)?$/;
		break;
		
		case 'disp':
			website_regex = /^(www\.)[a-zA-Z0-9]+([\-\.]{1}[a-zA-Z0-9]+)*\.[a-zA-Z]{2,3}(:[0-9]{1,5})?(\/.*)?$/;
		break;
	}
...
function campaigns_init () {
	...
	$('#campaign_list').on('change',function(){
		if ( $(this).val() != 0 ) {
			get_campaign( $(this).val(), false, false );
		}
		else {
			location.reload();
		}
	...
	$('#budget').on('change',function(event){
		...
		if ( validateNumber(value) > 0 ) {
			...
			$('#budget').parent().addClass('has-success');
		}
		else {
			$('#budget').parent().removeClass('has-success');
	...
	$('#max_cpv').on('change',function(event){
		
	    var value = $.trim( $(this).val() );  
		if ( validateNumber(value) > 0 ) {
			$('#max_cpv').parent().removeClass('has-error');
			$('#max_cpv').parent().addClass('has-success');
		}
		else {
			$('#max_cpv').parent().removeClass('has-success');
			$('#max_cpv').parent().addClass('has-error');
		}
...
function get_campaign ( id, copy, optimized ) {
	...
	$.ajax({
		...
		if ( data.exported == 1 ) {
			...
			$('#add_video_ads').prop('disabled',true);
			$('#display_url').prop('disabled',true);
			$('#destination_url').prop('disabled',true);
			$('#budget').prop('disabled',true);
			$('#max_cpv').prop('disabled',true);
			$('#start_date').prop('disabled',true);
			$('#end_date').prop('disabled',true);
			$('#delivery_method').prop('disabled',true);
			$('#mbm-type').prop('disabled',true);
			$('#mobile_bid_mdifier').prop('disabled',true);
			$('.list-language input[type="checkbox"]').prop('disabled',true);
			$('.list-countries input[type="checkbox"]').prop('disabled',true);
			$('.age_checkbox').prop('disabled',true);
			$('.gender_checkbox').prop('disabled',true);
			$('.target_checkbox').prop('disabled',true);
			$('.disabled-detector').hide();
			$('.video_ads_item button.btn-danger').prop('disabled',true);
			$('.video_ads_item').off();
		}
		else {
			$('#exported-note').hide();
			$('#file_name').prop('disabled',false);
			$('#add_video_ads').prop('disabled',false);
			$('#display_url').prop('disabled',false);
			$('#destination_url').prop('disabled',false);
			$('#budget').prop('disabled',false);
			$('#max_cpv').prop('disabled',false);
			$('#start_date').prop('disabled',false);
			$('#end_date').prop('disabled',false);
			$('#delivery_method').prop('disabled',false);
			$('#mbm-type').prop('disabled',false);
			$('#mobile_bid_mdifier').prop('disabled',false);
			$('.list-language input[type="checkbox"]').prop('disabled',false);
			$('.list-countries input[type="checkbox"]').prop('disabled',false);
			$('.age_checkbox').prop('disabled',false);
			$('.gender_checkbox').prop('disabled',false);
			$('.target_checkbox').prop('disabled',false);
			$('.disabled-detector').show();
			$('.video_ads_item button.btn-danger').prop('disabled',true);
			/*
$('.video_ads_item').on('click',function(){
				if($(this).hasClass('vid-ads-item-selected')) {
                	$('#' + $(this).data('option-id') ).attr('selected', false);
                	$(this).children('label').removeClass('glyphicon-ok');
                	$(this).children('label').addClass('list-indent');
...
*/
		...
		var checked = $('.target_checkbox:checked');
		if ( checked.length == 10 ) {
			$('.target_checkbox:not(:checked)').prop('disabled',true);
		}
		else if ( checked.length == 1 ) {
			if ( total_list > max_tgv ) {
				$('.target_checkbox:not(:checked)').prop('disabled',true);
			}
		}
		...
		$("#total_targets font").html(checked.length);
		if (total_list > 0) {
			$("#total_target_vid").show();
		}
		...
		if (!optimized) {
			...
			$('#saveCampaign').text('Save Optimized Campaign');
		}
		...
		$('#saveCampaign').data("exported", data.exported);
		$('#saveCampaign').attr("disabled", false);
		$('#saveCampaign').removeClass('btn-default');
		$('#saveCampaign').addClass('btn-success');
		myValidate();
...
function myValidate () {
	if($('#file_name').val().trim() != "" &&
		...
		$('.age_checkbox:checked').length > 0 &&
		$('.gender_checkbox:checked').length > 0 &&
		validateNumber($('#budget').val().trim()) > 0 &&
		validateNumber($('#max_cpv').val().trim()) > 0) {
			$('#btnExportCSV').attr("disabled", false);
			$('#btnExportCSV').removeClass('btn-default');
			$('#btnExportCSV').addClass('btn-primary');
...
*/
	...
	if ( $('#video_ads_list li.vid-ads-item-selected').length > 0 ) {
		...
		$('#video_ads_list').css('border', 'medium solid #a94442');
	}
...
*/
	...
	if ( validateNumber($('#budget').val()) > 0 ) {
		$('#budget').parent().removeClass('has-error');
		$('#budget').parent().addClass('has-success');
	}
	else {
	...
	if ( validateNumber($('#max_cpv').val()) > 0 ) {
		$('#max_cpv').parent().removeClass('has-error');
		$('#max_cpv').parent().addClass('has-success');
	}
	else {
	...
	/*
if ( $('#saveCampaign').data('exported') == 1 ) {
		$('#saveCampaign').attr("disabled", false);
	}
	else {
		$('#saveCampaign').attr("disabled", false);
...
*/
	...
	$('#first_export_reload').bind('click', function(){
		setTimeout(location.reload(true), 400);
	});
