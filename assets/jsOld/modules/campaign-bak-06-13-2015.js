...
$('#video_ads_list').css({'margin-top':'10px', 'height':'220px', 'width':'520px', 'padding':'0px', 'list-style' : 'none', 'overflow-y' : 'auto', 'background-color' : '#eee' });
	...
	$('.target_checkbox').on('click', function(){
        ...
        if ( checked > 0 ) {
        	if ( ytdata > max_tgv ) {
				if ( left > 1 ) {
	            	$('#target_limit_modal .modal-body p').html("This needs to be it’s OWN campaign, so untick all other lists and we’ll create an Adwords campaign JUST for this list.");
...
*/
	...
	$('#btnExportCSV').bind('click', function(){
		...
		$('.target_checkbox:checked').each(function(y, val){
			...
			var targetgroupname  = $(val).parent('label').text().replace(/\s+\(.+?\)/g, '');
			
			for(var j=0;j<video_ads.length;j++){
	
				if(video_ads[j]!=null){	
					//console.log($(video_ads_option[i]).data('title'));			
					var target_ads_content = "<tr class=\"table-data\"> <td>Add</td> <td>Enabled</td> <td>"+$(video_ads_option[j]).data('title')+"</td> <td>"+video_ads[j]+"</td> <td> default</td> <td>#N/A</td> <td> #N/A </td> <td>#N/A</td> <td>"+disp_url+"</td> <td>"+dest_url+"</td> <td>#N/A</td> <td>in-stream</td> <td>#N/A</td> <td>"+targetgroupname+"</td> <td>"+filename+"</td> <td colspan=\"41\"></td></tr>";
