...
thead {
  ...
  <h3>Send email to demo users</h3>   
  <div id="notification"></div>
  <p>
    <div class="panel panel-default">
      <!-- Default panel contents -->
      <div class="panel-heading">List of Demo users</div>
...
<table class="table table-striped">
    ...
    <tr>
        ...
        <td class="filterable-cell"><?php echo $su['email']; ?></td>
    </tr>
    <?php
      }
    ?>
...
<p style="padding-top:20px;"><button name="send" type="button" class="btn btn-success" id="submit_email_demo">Send</button> 
<button name="send" type="button" class="btn btn-danger" id="test_email_demo">Test Send Email</button></p>
</form>
        </div>
    </div> <!-- /row -->
</div> 
...
$('#submit_email_demo').click(function(e){
  ...
  if(subject.length < 5){
    $('#notification').html('<div class="alert alert-warning" role="alert"> <strong>Warning!</strong> subject atleast 5 characters</div>');
  ...
  }else if(data['checked[]'].length == 0)
  {
    $('#notification').html('<div class="alert alert-warning" role="alert"> <strong>Warning!</strong> Please select atleast 1 user</div>');
        ...
        ({
        type: "POST",
        url: baseurl + "emaildemo/send_email",
        cache: false,
        data: data,
        beforeSend: function()
        {
...
$('#test_email_demo').click(function(e){
  ...
  if(subject.length < 5){
    ...
    $('#notification').html('<div class="alert alert-warning" role="alert"> <strong>Warning!</strong> message atleast 5 characters</div>');
        ...
        ({
        type: "POST",
        url: baseurl + "emaildemo/demo_test_send_email",
        cache: false,
        data: data,
        beforeSend: function()
        {
        ...
        success: function(result)
        {
          ...
          $('#submit_email_demo').addClass('btn-primary'); 
          if(res[1] == 'success'){
            $('#notification').html('<div class="alert alert-success" role="alert"> Test Message successfully sent </div>');
          } else{
            $('#notification').html('<div class="alert alert-danger" role="alert"> <strong>Error!</strong> Failed to send email, please try again.</div>');
          }     
        }        
