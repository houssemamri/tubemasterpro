...
class Optimizer_ajax extends Ajax_Controller {
	...
	function get_campaign_info () {
		...
		$html .= '</div>';
		$html .= '<div class="col-sm-8">';
		$html .= '<span>'.date('M d Y',strtotime($campaign->start_date)).'</span>';
		$html .= '</div>';
		$html .= '</div>';
		//- End Date
		$html .= '<div class="row bg-info">';
		...
		if ( $campaign->end_date == '#N/A' ) {
			...
			$html .= '<span>'.date('M d Y',strtotime($campaign->end_date)).'</span>';
		}
		$html .= '</div>';
		$html .= '</div>';
		//- Delivery Method
...
*/
						...
						if ( strtolower($c_name) == strtolower($c_orig->name) ) {
							...
							$return['names']= strtolower($c_name) . ' == ' . strtolower($c_orig->name);
							$return['json'] = $xmlphp[0];
							$return['data'] = $adsData;
						}
