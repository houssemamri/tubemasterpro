...
*/
        ...
        if ( $targets ) {
            ...
            foreach ($targets as $key => &$value) {
                ...
                $row .= "<div class='form-group col-sm-2'>";
                $row .= "<button type='button' ".$get_links_class." title='' data-toggle='tooltip' data-placement='top' data-original-title='Get Links' data-links='".json_encode($links)."' >Get Links</button>";
                $row .= "<textarea class='links' style='display:none;'></textarea>";
                $row .= "</div>";
