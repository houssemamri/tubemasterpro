...
*/
        ...
        else if ( $this->ion_auth->in_group(3) || $this->ion_auth->in_group(2) ) //remove this elseif if you want to enable this for non-admins
        {
            //if has unlimited promo code
    ...
    public function campaign_upload ()
    {
        ...
        if ( $campaign_list ) {
            ...
            foreach ($campaign_list as $key => $value) {
                ...
                if ($uploaded) {
                    $campaigns .= '<tr>';
                    $campaigns .= '<td align="center"><input type="checkbox" name="to_delete" data-cid="'.$value->id.'" class="to_delete"></td>';
                    $campaigns .= '<td>'.$value->name.'</td>';
                    $campaigns .= '<td>'.date('Y-m-d h:i:s A',$uploaded->date_uploaded).'</td>';
                    $campaigns .= '<td class="text-center">';
                    $campaigns .= '<p>';
                    $campaigns .= '<button data-cid="'.$value->id.'" type="button" class="btn btn-warning btn-jumbo btn-info" style="font-size:0.9em"><span class="glyphicon glyphicon-info-sign"></span> Campaign Info</button> ';
                    $campaigns .= '<span class="btn btn-jumbo btn-success fileinput-button" style="font-size:0.9em"><i class="glyphicon glyphicon-upload"></i><input class="btn-upload" data-cid="'.$value->id.'" type="file" name="campaign_file" ><span> Update Campaign</span></span> ';
                    $campaigns .= '<button data-cid="'.$value->id.'" type="button" class="btn btn-danger btn-jumbo btn-delete" style="font-size:0.9em"><span class="glyphicon glyphicon-remove-sign"></span> Delete Campaign</button> ';
                    $campaigns .= '</p>';
                    $campaigns .= '</td>';
                    $campaigns .= '</tr>';
                }
                    ...
                    $campaigns .= '<p>';
                    $campaigns .= '<button data-cid="'.$value->id.'" type="button" class="btn btn-warning btn-jumbo btn-info" style="font-size:0.9em"><span class="glyphicon glyphicon-info-sign"></span> Campaign Info</button> ';
                    $campaigns .= '<span class="btn btn-jumbo btn-primary fileinput-button" style="font-size:0.9em"><i class="glyphicon glyphicon-upload"></i><input class="btn-upload" data-cid="'.$value->id.'" type="file" name="campaign_file" ><span> Upload Adwords CSV</span></span> ';
                    $campaigns .= '<button data-cid="'.$value->id.'" type="button" class="btn btn-danger btn-jumbo btn-delete" style="font-size:0.9em"><span class="glyphicon glyphicon-remove-sign"></span> Delete Campaign</button> ';
                    $campaigns .= '</p>';
                    $campaigns .= '</td>';
                    $campaigns .= '</tr>';
...
/*        foreach($shw_r->result_array() as $key=>$value){
    ...
    public function tube_university(){
        $this->template->set_title('Tube University');
        $this->load->database();
        $this->load->model('target_model');
        
        $active = 'tube_university';
        $parent = 'tube_university';
      ...
      public function video_production(){
        $this->template->set_title('Video Ad Production');
        $this->load->database();
        $this->load->model('target_model');
        
        $active = 'video_production';
    ...
    function adwords_notice_accepts(){
        $this->load->model('Logs_model', 'logs'); 
        setcookie("user_accept_ad_notice", 1, time() + (86400 * 30), "/"); // 86400 = 1 day

        $this->logs->insert_logs("Adwords Notice", "Accepted Adwords Emergency Notice - " . date("m/d/Y H:i a", time()));
        echo 1;
    }  


    function adwords_back_notice(){
        $this->load->model('Logs_model', 'logs'); 
        setcookie("adwords_back_notice", 1, time() + (86400 * 30), "/"); // 86400 = 1 day

        $this->logs->insert_logs("Adwords Notice - Campaign Builder", "Accepted Adwords Emergency Notice from campaign builder- " . date("m/d/Y H:i a", time()));
        echo 1;
    }  

    function reset_adwords_notice(){
        setcookie("user_accept_ad_notice", 0, time() + (86400 * 30), "/"); // 86400 = 1 day
        setcookie("adwords_back_notice", 0, time() + (86400 * 30), "/"); // 86400 = 1 day
